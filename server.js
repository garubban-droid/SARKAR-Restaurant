...
await pool.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivery_longitude DOUBLE PRECISION`);
  ...
  await pool.query(`ALTER TABLE customer_otps ADD COLUMN IF NOT EXISTS code_hash TEXT`);
  await pool.query(`UPDATE customer_otps SET code_hash=otp_hash WHERE code_hash IS NULL AND otp_hash IS NOT NULL`);
...
async function issueCustomerOtp({phone,mode,pendingData}){
  ...
  await q(`INSERT INTO customer_otps(phone,mode,otp_hash,code_hash,pending_data,attempts,expires_at)
