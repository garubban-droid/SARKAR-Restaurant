...
app.post('/api/customer/auth/verify-otp',authLimiter,async(req,res)=>{const phone=normalizePhone(req.body.phone),otp=String(req.body.otp||'');const r=db.prepare('SELECT * FROM otp_requests WHERE phone=? AND mode=? AND verified=0 ORDER BY created_at DESC LIMIT 1').get(phone,req.body.mode==='signup'?'signup':'login');if(!r||new Date(r.expires_at)<new Date())return res.status(400).json({error:'OTP expired or invalid'});if(r.attempts>=5)return res.status(429).json({error:'Too many OTP attempts'});db.prepare('UPDATE otp_requests SET attempts=attempts+1 WHERE id=?').run(r.id);if(!bcrypt.compareSync(otp,r.otp_hash))return res.status(400).json({error:'Invalid OTP'});db.prepare('UPDATE otp_requests SET verified=1 WHERE id=?').run(r.id);if(r.mode==='signup')return res.json({pendingSignup:true,signupToken:jwt.sign({phone,otpId:r.id,role:'signup'},JWT_SECRET,{expiresIn:'10m'})});const c=db.prepare('SELECT * FROM customers WHERE phone=?').get(phone);if(!c)return res.status(404).json({error:'Account not found'});res.json({token:signCustomer(c),customer:publicCustomer(c)});});
