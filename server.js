...
async function sendCustomerOtpSms(phone,otp){
  ...
  // 2Factor's manual OTP API uses the legacy V1 URL format.
  // We generate the OTP ourselves so our database can verify the same code.
  const targetPhone=`91${phone}`;
  const url=`https://2factor.in/API/V1/${encodeURIComponent(apiKey)}/SMS/${encodeURIComponent(targetPhone)}/${encodeURIComponent(otp)}/${encodeURIComponent(templateName)}`;
  ...
  if(!r.ok || status!=='success'){
    const detail=String(data.Details||data.details||data.message||data.error||raw||'Unknown 2Factor error').trim();
    throw new Error(`2Factor OTP failed (${r.status}): ${detail.slice(0,500)}`);
  }
  return data;
}
