...
const authLimiter=rateLimit({windowMs:10*60*1000,max:100,standardHeaders:true,legacyHeaders:false,message:{error:'Too many authentication attempts. Please try again later.'}});
...
app.post('/api/customer/auth/send-otp',authLimiter,async(req,res)=>{
  ...
  try{
    ...
    res.json({ok:true,message:'OTP sent',requestId,mode,existingCustomer:exists});
  }catch(e){
...
app.post('/api/customer/auth/verify-otp',authLimiter,async(req,res)=>{const phone=normalizePhone(req.body.phone),otp=String(req.body.otp||'');/* Always resolve the latest OTP for this phone, regardless of the UI mode. The server decides old-vs-new from the database. */const r=db.prepare('SELECT * FROM otp_requests WHERE phone=? AND verified=0 ORDER BY created_at DESC LIMIT 1').get(phone);if(!r||new Date(r.expires_at)<new Date())return res.status(400).json({error:'OTP expired or invalid'});if(r.attempts>=5)return res.status(429).json({error:'Too many OTP attempts'});db.prepare('UPDATE otp_requests SET attempts=attempts+1 WHERE id=?').run(r.id);if(!bcrypt.compareSync(otp,r.otp_hash))return res.status(400).json({error:'Invalid OTP'});db.prepare('UPDATE otp_requests SET verified=1 WHERE id=?').run(r.id);/* Final safety check: if the phone belongs to an existing customer, OTP always logs them in directly. This prevents an old number from ever reaching the signup profile screen due to stale/racing state. */const existing=db.prepare('SELECT * FROM customers WHERE phone=?').get(phone);if(existing)return res.json({token:signCustomer(existing),customer:publicCustomer(existing),mode:'login',existingCustomer:true});if(r.mode==='signup')return res.json({pendingSignup:true,signupToken:jwt.sign({phone,otpId:r.id,role:'signup'},JWT_SECRET,{expiresIn:'10m'}),mode:'signup',existingCustomer:false});return res.status(404).json({error:'Account not found'});});
