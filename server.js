...
async function sendCustomerOtpSms(phone,otp){
  ...
  // SMS-ONLY customer OTP.
  // The OTP is generated here so the same code can be verified by our database.
  ...
  if(!r.ok || status!=='sent'){
    const detail=String(
      ...
      data.reason||data.Reason||raw||'Unknown 2Factor SMS error'
    ).trim();
    throw new Error(`2Factor SMS OTP failed (${r.status}): ${detail.slice(0,500)}`);
  }
