...
await pool.query(`ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivery_longitude DOUBLE PRECISION`);
  ...
  const defaultReferralSettings={
    ...
    spamDetection:true,spamThreshold24h:3,codePrefix:'MIEXPRESS',maxReferralsPerCustomer:null
  };
  ...
  async function ensureReferralCode(customerId, phone){
    for(let i=0;i<8;i++){
      const code=`MIEXPRESS${crypto.randomInt(100000,999999)}`;
      try{
...
const DEFAULT_REFERRAL_SETTINGS={enabled:true,rewardType:'PROMOTIONAL_BALANCE',rewardAmount:50,minOrderAmount:0,qualifyingEvent:'FIRST_ORDER_DELIVERED',blockSameDevice:true,flagMultipleAccounts:true,spamDetection:true,spamThreshold24h:3,codePrefix:'MIEXPRESS',maxReferralsPerCustomer:null};
async function getReferralSettings(){
  const r=await q('SELECT data FROM referral_settings WHERE id=1');
  return {...DEFAULT_REFERRAL_SETTINGS,...(r.rows[0]?.data||{})};
}
async function makeReferralCode(prefix='MIEXPRESS'){
  const p=String(prefix||'MIEXPRESS').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,12)||'MIEXPRESS';
  for(let i=0;i<20;i++){
...
app.put('/api/admin/referral-settings',auth,async(req,res)=>{
  ...
  const data={
    ...
    codePrefix:String(x.codePrefix||'MIEXPRESS').toUpperCase().replace(/[^A-Z0-9]/g,'').slice(0,12)||'MIEXPRESS',
...
app.get('/api/settings',async(_,res)=>{const r=await q('SELECT data FROM settings WHERE id=1');const data=normalizeCustomerProfileSettings(r.rows[0]?.data||{});if(['সরকার রেস্টুরেন্ট & বিরিয়ানি','বিরিয়ানি এক্সপ্রেস','SARKAR RESTAURANT & BIRYANI','BIRYANI EXPRESS'].includes(data.brandBn)||['সরকার রেস্টুরেন্ট & বিরিয়ানি','বিরিয়ানি এক্সপ্রেস','SARKAR RESTAURANT & BIRYANI','BIRYANI EXPRESS'].includes(data.brandEn)){data.brandBn='MI EXPRESS BIRYANI';data.brandEn='MI EXPRESS BIRYANI'}res.json(data)});
...
async function startServer(){
  try{
    await initDatabase();
    app.listen(port,()=>console.log(`MI EXPRESS BIRYANI production server listening on :${port}`));
  }catch(error){
