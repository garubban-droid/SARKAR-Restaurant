...
app.use((req,res,next)=>{
  res.setHeader('Permissions-Policy','web-share=(self), geolocation=(self)');
  next();
});
...
const DEFAULT_REFERRAL_SETTINGS={enabled:true,rewardType:'PROMOTIONAL_BALANCE',rewardAmount:50,minOrderAmount:0,qualifyingEvent:'FIRST_ORDER_DELIVERED',blockSameDevice:true,flagMultipleAccounts:true,spamDetection:true,spamThreshold24h:3,codePrefix:'SARKAR',maxReferralsPerCustomer:null,referralTitle:'Rewards & Referral',referralDescription:'বন্ধুদের রেফার করুন এবং যোগ্য referral সম্পন্ন হলে reward পান।',referralSteps:[{titleBn:'রেফারেল লিংক শেয়ার করুন',titleEn:'Share Your Link',descriptionBn:'বন্ধুদের আমাদের খাবার অর্ডার করতে আমন্ত্রণ জানান',descriptionEn:'Invite your friends to try our food'},{titleBn:'বন্ধু প্রথম অর্ডার করবে',titleEn:'Friend Places First Order',descriptionBn:'তাদের প্রথম যোগ্য অর্ডার Delivered হলে',descriptionEn:'When their first eligible order is delivered'},{titleBn:'পুরস্কার পান',titleEn:'Get Your Reward',descriptionBn:'যোগ্য referral সম্পন্ন হলে reward আপনার অ্যাকাউন্টে জমা হবে',descriptionEn:'Your reward is credited when the referral qualifies'}],referralTermsBn:'• Reward পেতে referred customer-এর প্রথম eligible order Delivered হতে হবে।\n• Minimum order condition প্রযোজ্য হতে পারে।\n• একই device বা spam/fraud activity হলে referral বাতিল হতে পারে।',referralTermsEn:'• The referred customer must complete the first eligible order.\n• A minimum order condition may apply.\n• Referrals may be rejected for same-device, spam or fraud activity.'};
...
app.get('/api/customer/referral',customerAuth,async(req,res)=>{
  try{
    ...
    res.json({enabled:settings.enabled,referralCode:code,referralLink:`${base}/?ref=${encodeURIComponent(code)}`,rewardType:settings.rewardType,rewardAmount:Number(settings.rewardAmount||0),minOrderAmount:Number(settings.minOrderAmount||0),referralTitle:settings.referralTitle,referralDescription:settings.referralDescription,referralSteps:settings.referralSteps,referralTermsBn:settings.referralTermsBn,referralTermsEn:settings.referralTermsEn,stats:stats.rows[0],promotionalBalance:Number(wallet.rows[0]?.promotional_balance||0),referrals:list.rows});
...
app.put('/api/admin/referral-settings',auth,async(req,res)=>{
  ...
  const data={
    ...
    referralTitle:String(x.referralTitle||DEFAULT_REFERRAL_SETTINGS.referralTitle).trim().slice(0,120),
    referralDescription:String(x.referralDescription||DEFAULT_REFERRAL_SETTINGS.referralDescription).trim().slice(0,500),
    referralSteps:Array.isArray(x.referralSteps)?x.referralSteps.slice(0,6).map((v,i)=>({titleBn:String(v?.titleBn||'').trim().slice(0,160),titleEn:String(v?.titleEn||'').trim().slice(0,160),descriptionBn:String(v?.descriptionBn||'').trim().slice(0,300),descriptionEn:String(v?.descriptionEn||'').trim().slice(0,300)})):DEFAULT_REFERRAL_SETTINGS.referralSteps,
    referralTermsBn:String(x.referralTermsBn||'').trim().slice(0,3000),
    referralTermsEn:String(x.referralTermsEn||'').trim().slice(0,3000)
  };
